...
subroutine bc_send
    ...
    !! SEND/RECV
    if (rank > 0 ) then
        call MPI_Irecv(r1mat,ghostPoints*3,MPI_DOUBLE_PRECISION,rank-1,0,MPI_COMM_WORLD,recvLEFT,ierr)

    end if


    if (rank <nprocs-1) then
        call MPI_Irecv(r2mat,ghostPoints*3,MPI_DOUBLE_PRECISION,rank+1,1,MPI_COMM_WORLD,recvRIGHT,ierr)
    ...
    do j = 1,ghostPoints
        ...
        !s1mat(:,j) = U(:,localLOW+j,2)
        !s2mat(:,j) = U(:,localHIGH-j-1,2)
    end do



    if (rank > 0 ) then
        call MPI_Isend(s2mat,ghostPoints*3,MPI_DOUBLE_PRECISION,rank-1,1,MPI_COMM_WORLD,sendLEFT,ierr)
    end if


    if (rank <nprocs-1) then
        call MPI_Isend(s1mat,ghostPoints*3,MPI_DOUBLE_PRECISION,rank+1,0,MPI_COMM_WORLD,sendRIGHT,ierr)
    end if
...
subroutine update(flag)
    ...
    !! FLUX CALCULATIONS 
    do i = localLOW-npoints, localHIGH+npoints            

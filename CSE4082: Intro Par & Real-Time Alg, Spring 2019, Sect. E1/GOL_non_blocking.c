...
int main(int argc, char* argv[]) {
  ...
  for (stepCounter=0; stepCounter < NumGenerations; stepCounter++)
  {
      ...
      /* Set up MPI_request */ 
       ...
       if (myid >0) {
		MPI_Irecv(uOld[0],N+2,MPI_INT,myid-1,0,MPI_COMM_WORLD,&requestTOP);
      		 MPI_Isend(uOld[0],N+2,MPI_INT,myid-1,1,MPI_COMM_WORLD,&sentTOP);
	}


	if (myid <numprocs-1){

       		MPI_Isend(uOld[Nlocal+1],N+2,MPI_INT,myid+1,0,MPI_COMM_WORLD,&sentBOTTOM);
       		MPI_Irecv(uOld[Nlocal+1],N+2,MPI_INT,myid+1,1,MPI_COMM_WORLD,&requestBOTTOM);
       ...
       if (myid != 0){
		...
		MPI_Wait(&sentTOP,MPI_STATUS_IGNORE);
       ...
       if (myid != numprocs-1){
		MPI_Wait(&sentBOTTOM,MPI_STATUS_IGNORE);
